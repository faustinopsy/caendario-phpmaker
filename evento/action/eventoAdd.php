...
//Seleciona ultimo evento e incrementa a tabela 'convites' se necessario
		$existeEvento = "SELECT * FROM eventos where DATE_FORMAT(inicio, 'Y/m/d H:i:s') BETWEEN $inicio  AND $termino   OR  DATE_FORMAT(termino, 'Y/m/d H:i:s')  BETWEEN '$inicio' AND '$termino' ";
		$req = $db->prepare($existeEvento);
		$req->execute();
		$linhas = $req->rowCount();
		if ($linhas >= 1) {
			 die ('Erro ao Evento duplicado');
		print_r ("Erro ao carregar");
		} else{
