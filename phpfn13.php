...
function CurrentProjectID() {
	...
	return "{9661832A-1868-435B-AC02-F5871FC25E94}";
}
...
class cAdvancedSecurity {
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE) {
		...
		if (!$validateUser) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$validateUser = $customValidateUser || ew_ComparePassword($rs->fields('senha'), $pwd, $encrypted);
					if ($validateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('nome')); // Load user name
						$this->setSessionUserID($rs->fields('id_usuario')); // Load User ID
	...
	function GetUserIDByUserName($UserName) {
		...
		if (strval($UserName) <> "") {
			...
			$sSql = $UserTable->GetSQL($sFilter, '');
			if (($RsUser = $UserTableConn->Execute($sSql)) && !$RsUser->EOF) {
				$UserID = $RsUser->fields('id_usuario');
				$RsUser->Close();
				return $UserID;
			}
		}
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $UserTableConn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('id_usuario'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
